...
public class PlayerDirtyTalkComponent : MonoBehaviour {
    ...
    static PlayerDirtyTalkComponent() {
        ClassInjector.RegisterTypeInIl2Cpp<PlayerDirtyTalkComponent>();
    }

    public PlayerDirtyTalkComponent() : base(ClassInjector.DerivedConstructorPointer<PlayerDirtyTalkComponent>()) {
    ...
    public void Initialize() {
        if (gameObject.TryGetComponentWithCast(out PlayerCombat _)) {
            ...
            Destroy(this);
            Plugin.Log.Info("PlayerComponent Destroyed");
        }
    }
    ...
    public void Initialise() {
        ...
        if (sexSystem is not null) {
            ...
            if (_sexSystem.PlayerMale) {
                ...
                foreach (var enemyType in PlayerDirtyTalkMod.PlayerEnemyTypes) {
    ...
    #region Idle
    public void Idle() {
        var dirtyTalkModel = GetDirtyTalkModel();
    ...
    public void PlayerGrapple() {
        ...
        var (dirtyTalkModel, enemyGender) = GetDirtyTalkModel(enemyType);
        if (dirtyTalkModel is null)
            return;
    ...
    public static void RegisterClass(PlayerCombat playerCombat) {
        playerCombat.gameObject.AddComponentWithAction<PlayerDirtyTalkComponent>(component => component.Initialize());
    }
}