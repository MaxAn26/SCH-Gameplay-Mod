...
internal class SexSystemPatch {
    ...
    static void SexSystemGrappleBeforeSexPostfix(SexSystem __instance) {
        __instance.Enemy?.GetComponentWithCast<EnemyDirtyTalkComponent>()?.EnemyGrapple();
    }

    [HarmonyPostfix]
    [HarmonyWrapSafe]
    [HarmonyPatch(typeof(SexSystem), nameof(SexSystem.PlayerGrappleBeforeSex))]
    static void SexSystemPlayerGrappleBeforeSexPostfix(SexSystem __instance) {
        __instance.Player.GetComponentWithCast<PlayerDirtyTalkComponent>()?.PlayerGrapple();
    }
}