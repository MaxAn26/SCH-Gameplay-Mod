...
internal class EnemySexExtendMod {
    ...
    internal static ConfigEntry<bool> UseEnemyThreesomeChance;
    #endregion
    ...
    internal static void Load(ConfigFile config) {
        try {
            Enabled = config.Bind(nameof(EnemySexExtendMod), nameof(Enabled), false,
                new ConfigDescription("Activates the modification", new AcceptableValueList<bool>([true, false])));
            UseEnemyThreesomeChance = config.Bind(nameof(SexMoveChoiceMod), nameof(UseEnemyThreesomeChance), false,
                new ConfigDescription("Use threesome chance according with enemy type", new AcceptableValueList<bool>([true, false])));


            if (Enabled.Value) {
                if (!JsonUtils.TryDeserialize(Plugin.PluginResources, "EnemySexTypes.json", out List<EnemySexModel> enemySexType)) {
