...
internal class LustCageMod
{
    ...
    internal static MelonPreferences_Entry<bool> DenyEscape;
    ...
    internal static void Load(ModConfig config)
    {
        try
        {
            Enabled = config.Entry(nameof(LustCageMod), nameof(Enabled), false,
                "Activates the modification", new AcceptableValueList<bool>([true, false]));
            DenyEscape = config.Entry(nameof(LustCageMod), nameof(DenyEscape), true,
                "Player can't escape from interaction and should end it by cumming or using counter", new AcceptableValueList<bool>([true, false]));
        ...
        try
        {
            ...
            switch (sexSystem.SexType)
            {
                case 1: // oral
                    ...
                    {
                        bool active = SexSystem.PlayerAttacker ? SexSystem.SexIsLickingTarget || SexSystem.SexIsOralTarget : SexSystem.SexIsLickingCaster || SexSystem.SexIsOralCaster;
                        if (active)
                        {
                    ...
                    if (config.LickPussy)
                    {
                        bool active = SexSystem.PlayerAttacker ? SexSystem.SexIsLickingCaster : SexSystem.SexIsLickingTarget;
                        if (active)
                        {
                            ActivateBuff(sexSystem);
                            return;
                    ...
                    if (config.SuckDick)
                    {
                        bool active = SexSystem.PlayerAttacker ? SexSystem.SexIsOralCaster : SexSystem.SexIsOralTarget;
                        if (active)
                        {
                            ActivateBuff(sexSystem);
                            return;
    ...
    internal static bool BlockEscape(SexSystem sexSystem) => SceneManager.GetActiveScene().buildIndex > 4 && DenyEscape.Value && sexSystem.playerweak;
